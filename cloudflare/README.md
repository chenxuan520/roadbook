...
**部署完成后：**
1.  您将获得一个 `https://roadbook-backend.<your-subdomain>.workers.dev` (或类似) 的地址。
2.  **直接访问该地址**，即可看到 Roadbook 前端界面。
3.  默认管理员账号：`admin`，密码：`password`。
...
3.  **创建 KV 命名空间**：
    ...
    > **⚠️ 重要提示：务必手动回填 ID**
    >
    > Cloudflare Wrangler 部署时遵循“以本地配置为准”原则。
    > *   如果您**没有**将生成的 `id` 填入本地 `wrangler.toml` 就再次运行部署。
    > *   Wrangler 会认为您想要绑定一个新的或不同的 KV，可能会导致线上服务连接到错误的空数据库，造成数据“丢失”假象。
    >
    > **如果忘记了 ID**：运行 `npx wrangler kv:namespace list` 即可找回。

4.  **修改配置 (可选)**：
    打开 `wrangler.toml`（推荐修改项目根目录的那个，用于一键部署），在 `[vars]` 部分修改环境变量，如 `JWT_SECRET`。
