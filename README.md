...
### 📱 极致体验
- **高效的键盘工作流**: 提供丰富的快捷键（如 A/C/D/F）与撤销（Ctrl+Z）功能，实现行云流水般的操作体验。
- **一键系统集成**: 集成携程购票、多种地图导航（百度、高德、腾讯、Google）
- **开源免费**: 代码完全开源，可自由使用和修改

### 🏠 自托管优势
- **Serverless一键部署**: 支持 Cloudflare Workers 一键部署前后端，免费且无需维护服务器
- **Docker快速部署**: 支持Docker容器化部署，操作简单，适合个人服务器
...
- **图片导出（长图 PNG）**: 生成可分享的行程长图（含每日地图与时间轴），适合发朋友圈/群聊
- **ICS导出**: 生成符合iCalendar标准的.ics日历文件，支持导入到Apple日历、Google日历、Outlook等，实现行程提醒
...
- **快捷指令**:
  ...
  - `/xiaohongshu [提示词]`: 将当前行程导出为小红书文案（可选风格偏好）。
  - `/clear`: 清空对话历史。
  - `/help`: 查看帮助。
- **配置要求**: 此功能需要自托管，并在后端配置中提供 AI 模型（如 OpenAI、Cloudflare Workers AI 等）的 API Key。

### PWA 支持
- **应用安装**: 支持将网页安装为桌面或手机原生应用
- **离线访问**: 通过 Service Worker 缓存核心资源，弱网或离线环境下也能秒开
- **沉浸体验**: 全屏运行，去除浏览器地址栏干扰，提供类原生 App 的操作体验

### 用户体验
- **快捷键支持**: A(添加)、C(连接)、D/Backspace/Del(删除)、F(调整视窗)、H(帮助)等；在线模式下支持 Ctrl/Cmd+S 触发“云端保存”
- **撤销功能**: Ctrl+Z支持撤销操作
...
> **✨ 一键部署前后端**
> 
> 点击下方的按钮，即可将包含完整前端（静态资源代理）和后端（API）的 Roadbook 服务部署到您的 Cloudflare Workers。
>
...
### 导出分享
1. 点击"导出路书"按钮
2. 选择"导出JSON"/"导出HTML"/"导出图片"
3. JSON文件可用于备份和后续编辑
4. HTML文件可独立分享，包含完整地图

#### 图片导出（长图 PNG）说明
- **内容**：封面（标题/计划总览）+ 行程总览地图 + 每日地图 + 时间轴
- **时间轴规则**：按连接线拓扑生成链路；同一天出现多条链路（多人行程/岔路）会分开展示为“行程 1/2/…”
- **交通备注/价格显示**：连接线的 `label` 会以标签形式显示，便于多条行程时快速区分
- **点位图片链接（logo）**：若点位配置了图片链接，导出会尝试渲染；加载失败会提示并跳过；同一个点跨多天重复出现时仅首次渲染一次
- **地图瓦片加载**：导出前最多等待 15 秒；若仍有瓦片未加载，仍会导出但会弹提示“地图可能不完整”（空白区域）
- **暗黑模式**：导出卡片/文字会随明暗主题变化；地图暗色效果取决于瓦片源与浏览器对 CSS `filter` 的支持，可能与主界面略有差异

### PWA 安装指南 (手机 App 体验)
RoadbookMaker 支持 PWA (Progressive Web App)，您可以将其安装到手机桌面，获得类似原生 App 的体验。

**iOS (Safari 浏览器):**
1. 在 Safari 中打开 RoadbookMaker 网页
2. 点击底部的"分享"按钮 (带箭头的方框图标)
3. 向下滑动并选择"添加到主屏幕"
4. 点击右上角的"添加"

**Android (Chrome 浏览器):**
1. 在 Chrome 中打开 RoadbookMaker 网页
2. 点击右上角的菜单按钮 (三个点)
3. 选择"安装应用"或"添加到主屏幕"
4. 按照提示确认安装
