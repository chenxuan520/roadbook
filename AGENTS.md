...
- **语法检查**：每次修改代码后，必须自行检查语法（Go 语言需要检查 unused imports 等编译错误），确保代码可编译/可运行。
...
- **配置**：
  ...
  - 在 Cloudflare Dashboard 绑定 AI 模块（Binding 名称：`AI`），并设置环境变量 `USE_CF_AI=true` 即可启用免费的内置大模型（默认：@cf/qwen/qwen3-30b-a3b-fp8）。
- **维护规则（重要）**：
