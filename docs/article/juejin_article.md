...
*   **🏠 灵活的部署方式**
    ...
    *   **Serverless支持**: 新增 **Cloudflare Worker** 版本，无需服务器，利用免费额度即可搭建全功能后端。
    *   **数据私有**: 自托管意味着数据完全由你掌控，隐私安全感拉满。

## 新特性：不仅仅是离线工具

随着版本的迭代，RoadbookMaker 已经进化为一个“双模”工具：

*   **☁️ 在线模式 (Online Mode)**: 虽然主打离线，但也支持自建后端。配置好后，你可以**注册登录**，将路书**云端同步**，并在多设备间无缝切换。
*   **🔗 一键分享**: 在线模式下，无需导出文件，直接生成一个**分享链接**发给朋友，对方打开即看（支持密码保护）。
*   **🌍 多地图源聚合**: 除了默认的 OpenStreetMap，现已完美支持**高德、百度、腾讯、Google Maps**及**卫星图**。更贴心的是，搜索功能会自动**测速**（⚡️），帮你自动选择当前网络下最快的地图服务商。
...
*   **连接标记点**：点击“连接标记点”按钮或按 `c`，在弹出的窗口中选择起始点和目标点。
*   **⚡️ 高手技巧**：直接在地图上**按住鼠标右键拖拽**，从一个点拖到另一个点，即可秒级创建连线！系统还会根据距离自动推荐是“步行”、“驾车”还是“飞机”。
...
为了应对复杂的长途旅行规划，我还加入了一些“生产力”功能：

*   **📅 日期范围筛选**：行程太长看着眼花？用右上角的日期筛选器，只看某一天的行程，专注当下。
*   **↩️ 撤销与重做**：手滑删错了点？别慌，`Ctrl+Z` (撤销) 和 `Ctrl+Y` (重做) 随时待命，给你无限的后悔药。
*   **🌗 明暗模式**：深夜做攻略太刺眼？系统会自动跟随你的设备切换到**深色模式**，保护视力。

### 第五步：分享与备份你的路书

规划完成后，你可以轻松地将成果分享给朋友或进行备份。

*   **在线链接分享**：(在线模式) 直接复制浏览器地址栏的链接，朋友打开就能看。
...
*   **后端 (Go + Gin)**: 后端采用Go语言开发，主要为了实现高性能的API服务和极简的部署。Go可以将整个应用编译成一个无依赖的二进制文件，这使得Docker镜像可以做到非常小。同时，我们也提供了 **Cloudflare Worker** 版本，通过 Serverless 技术进一步降低部署成本。
